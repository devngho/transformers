...
class FlaxMistralMLP(nn.Module):
    ...
    def setup(self):
        ...
        self.down_proj = nn.Dense(embed_dim, use_bias=False, dtype=self.dtype, param_dtype=self.dtype, kernel_init=kernel_init)
...
class FlaxMistralAttention(nn.Module):
    ...
    def __call__(
        ...
        query_states = jnp.reshape(query_states, query_states.shape[:2] + (self.num_heads, self.head_dim))
