...
from huggingface_hub.utils import is_numpy_available

from .configuration_utils import PretrainedConfig
from .utils import is_flax_available, logging


logger = logging.get_logger(__name__)


if is_flax_available() and is_numpy_available():
    import jax
...
def _compute_default_rope_parameters(
    ...
    **rope_kwargs,
    ...
    return jnp.array(inv_freq), attention_factor
...
def _compute_yarn_parameters(
    ...
    def linear_ramp_factor(min, max, dim):
        ...
        linear_func = (np.arange(dim) - min) / (max - min)
...
def _compute_longrope_parameters(
    ...
    if expanded_max_position_embeddings > max_position_embeddings:
        ext_factors = np.array(long_factor)
    else:
        ext_factors = np.array(short_factor)
...
def _compute_llama3_parameters(
    ...
    return jnp.array(inv_freq_llama), attention_factor


# This maps the "rope_type" string field in rope config to the corresponding function to compute the RoPE parameters
# from the model config. You can append new {'rope_type': callable} pairs to this dictionary to enable custom RoPE
# parameterizations, as long as the callable has the same signature.
FLAX_ROPE_INIT_FUNCTIONS = {
...
FLAX_ROPE_VALIDATION_FUNCTIONS = {
...
def rope_config_validation(config: PretrainedConfig):
    ...
    validation_fn = FLAX_ROPE_VALIDATION_FUNCTIONS.get(rope_type)
