...
if is_flax_available():
    ...
    from transformers import FLAX_ROPE_INIT_FUNCTIONS
    from transformers.modeling_flax_rope_utils import rope_config_validation


@require_flax
class FlaxRopeTest(unittest.TestCase):
    # from torch.testing.assert_close `rtol` and `atol` for float32
    rtol = 1.3e-6
    atol = 1e-5

    def test_rope_validation(self):
        config = LlamaConfig()
        all_rope_types = FLAX_ROPE_INIT_FUNCTIONS.keys()
    ...
    def test_default_rope_function_bc(self):
        ...
        rope_fn = FLAX_ROPE_INIT_FUNCTIONS["default"]
        config_freqs = rope_fn(config=config)[0]
        kwargs_freqs = rope_fn(**rope_kwargs)[0]

        self.assertTrue(np.allclose(config_freqs, kwargs_freqs))
    ...
    def test_linear_rope_function_bc(self):
        ...
        rope_fn = FLAX_ROPE_INIT_FUNCTIONS["linear"]
        config_freqs = rope_fn(config=config)[0]
        kwargs_freqs = rope_fn(**rope_kwargs)[0]
    ...
    def test_dynamic_rope_function_bc(self):
        ...
        rope_fn = FLAX_ROPE_INIT_FUNCTIONS["dynamic"]
        config_freqs = rope_fn(config=config)[0]
        kwargs_freqs = rope_fn(**rope_kwargs)[0]
    ...
    def test_default_rope_numerically(self):
        ...
        self.assertTrue(jnp.allclose(inv_freq, EXPECTED_INV_FREQ, self.rtol, self.atol))
    ...
    def test_linear_rope_numerically(self):
        ...
        config = LlamaConfig()
        default_rope_fn = FLAX_ROPE_INIT_FUNCTIONS["default"]
        ...
        for factor in (2.0, 10.0, 20.0):
            ...
            self.assertTrue(jnp.allclose(inv_freq, default_inv_freq / factor, self.rtol, self.atol))
    ...
    def test_dynamic_rope_numerically(self):
        ...
        for factor in (2.0, 10.0, 20.0):
            ...
            self.assertTrue(jnp.allclose(inv_freq, default_inv_freq))
        ...
        self.assertFalse(jnp.allclose(inv_freq, default_inv_freq / factor))
        self.assertTrue(jnp.allclose(inv_freq, EXPECTED_INV_FREQ, self.rtol, self.atol))
    ...
    def test_yarn_rope_numerically(self):
        ...
        rope_fn = FLAX_ROPE_INIT_FUNCTIONS["yarn"]
        for factor in (2.0, 10.0, 20.0):
        ...
        self.assertTrue(jnp.allclose(inv_freq[-20:], default_inv_freq[-20:] / factor))
    ...
    def test_longrope_rope_numerically(self):
        ...
        long_factor = jnp.ones(dim // 2).cumsum(0).tolist()  # scaling applied when factor > 1.0
        ...
        rope_fn = FLAX_ROPE_INIT_FUNCTIONS["longrope"]
        max_position_embeddings = config.max_position_embeddings
        for factor in (2.0, 10.0, 20.0):
            config.rope_scaling = {
                "rope_type": "longrope",
        ...
        inv_freq, _ = rope_fn(config=config)
        self.assertTrue(jnp.allclose(inv_freq, default_inv_freq / jnp.array(short_factor)))
        ...
        inv_freq, _ = rope_fn(config=config)
        self.assertTrue(jnp.allclose(inv_freq, default_inv_freq / jnp.array(long_factor)))
    ...
    def test_llama3_rope_numerically(self):
        ...
        rope_fn = FLAX_ROPE_INIT_FUNCTIONS["llama3"]
        for factor in (2.0, 10.0, 20.0):
            config.rope_scaling = {
                "rope_type": "llama3",
                "factor": factor,
